...
![Results tab for a piece of research into DfE workforce. The figures are pulled in from the calculations tab, and then used to create a chart. Auto-checks are implemented to ensure that rows add up to expected number (i.e. figures are being pulled in correctly).](pictures/results.png "An example of a results tab.")
...
<center>
![Trying to do several different calculations in one go can lead to some complicated formulae.](pictures/formula_complicated.png){width=80%}

![Splitting calculations up into steps makes the process much easier to follow.](pictures/formula_easy.png){width=80%}
</center>
...
<center>
![Using nested IF statements to calculate to maximum change between two years can result in a complicated formula.](pictures/nested_if_complicated.png){width=80%}

![There is an easier way to do the same thing using the MAX and ABS functions.](pictures/nested_if_easy.png){width=80%}
